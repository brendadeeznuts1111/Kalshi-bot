...
export const PROBES: Record<string, GroundProbe[]> = {
  ...
  "format:probe": [
    ...
    p("fmt-8", "Bun.XML odds-heat", "odds feed contract: clusters/prints + american price + identity-guarded merge + singleton collapse", async () => {
      ...
      const e0 = events[0];
      const price = e0?.bookmakers?.[0]?.markets?.[0]?.outcomes?.[0]?.price ?? 0;
      return events.length === 2 && price === -150 ? ok("2 clusters, american -150 preserved") : bad(JSON.stringify({ n: events.length, price }));
    }),
