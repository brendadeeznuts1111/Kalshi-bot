...
export const LEDGER: LedgerClaim[] = [
  ...
  { id: 'IN-envColor', api: 'colors', source: 'bun.d.ts', fragment: 'Whether to colorize the output', docSays: 'CORRECTED proposal item 3: explicit colors:true + markdown.ansi emit ANSI even PIPED (env ignored); Bun.color(hex, "ansi") is AUTO - EMPTY when piped, honors NO_COLOR/FORCE_COLOR only at process START. Caller must gate env, Bun.inspect will not', evidencePath: 'inspectGotchas.inspectAnsiWhenPiped', kind: 'consistent' },
];
