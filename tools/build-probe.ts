...
try {
  ...
  await Bun.write(join(dir, "entry.ts"), "import { feature } from \"bun:bundle\";\nif (feature(\"SUPER_SECRET\")) { console.log(\"" + SECRET + "\"); }\nconsole.log(\"ALWAYS\");\n");
  ...
  await Bun.write(join(dir, "guard.ts"), "import { feature } from \"bun:bundle\";\nconsole.log(feature(\"X\"));\n");
  ...
  await Bun.write(join(dir, "rt.ts"), "import { feature } from \"bun:bundle\";\nif (feature(\"X\")) { console.log(\"ON\"); } else { console.log(\"OFF\"); }\n");
  ...
  await Bun.write(join(dir, "disk.ts"), "export const x = 1;\n");
  ...
  await Bun.write(join(dir, "dec.ts"), "function logged(value: any, ctx: any) { if (ctx.kind === \"method\") return function (...a: any[]) { console.log(\"called \" + ctx.name); return value.apply(this, a); }; return value; }\nclass C { @logged greet() { return \"hi\"; } }\nconsole.log(new C().greet());\n");
  ...
  await Bun.write(join(dir, "idx.html"), "<!doctype html><html><body><script>document.title = \"inline-ok\";</script></body></html>");
  ...
  await Bun.write(join(dir, "tla.ts"), "const v = await Promise.resolve(42);\nconsole.log(\"tla:\" + v);\n");
