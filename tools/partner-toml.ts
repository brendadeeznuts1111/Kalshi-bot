...
async function main(): Promise<void> {
  ...
  const wantEnv = hasFlag('check-env') || dryRun || hasFlag('validate') || hasFlag('seed');
  const wantAssets = hasFlag('check-assets') || dryRun || hasFlag('validate') || hasFlag('seed');
  ...
  try {
    ...
    if (wantSeed) {
      ...
      console.error(`seeded from ${path}: partners=${n.partners} outs=${n.accounts}`);
    ...
    if (hasFlag('json')) {
      ...
      console.log(`  version=${loaded.doc.version ?? '?'} title=${loaded.doc.title ?? '—'}`);
      console.log(`  partners=${loaded.partners.length} outs=${loaded.accounts.length}`);
      for (const p of loaded.partners) {
        const code =
          (loaded.doc.partners ?? []).find(
            x => x.id === p.id || `partner-${String(x.code).toLowerCase()}` === p.id
          )?.code ?? p.id;
      ...
      for (const a of loaded.accounts) {
        ...
        const bookBit = a.bookId ? `  book=${a.bookId}` : '';
        const skinBit = a.skinId ? `  skin=${a.skinId}` : '';
        console.log(
          `    └── ${a.id}  ${a.provider}  capacity=$${a.maxStake}  env=${a.envPrefix ?? '—'}${bookBit}${skinBit}  vault=${vault}`
        );
      }
