...
export function findSourceViolations(
  ...
  for (const item of imports) {
    ...
    const replacement = BANNED_PACKAGES.get(dependency);
    if (replacement && !SPAWN_KEEP_LIST.has(file)) {
      violations.push({
        file,
...
export const SPAWN_KEEP_LIST = new Set([
  ...
  "tests/lib/deps-tree.test.ts", // spawns `bun pm ls --all` (blocking dependency-shape gate)
  "tests/lib/odds-tile.test.ts", // spawns the odds-tile CLI (blocking end-to-end test)
  ...
  "tools/serve-tls-probe.ts", // openssl cert-gen + h2 fetch subprocess (pipe capture)
