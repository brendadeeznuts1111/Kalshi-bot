...
describe('partner registry', () => {
  ...
  test('seed stamps skinId=buckeye and bookId=fantasy402 from host', () => {
    ...
    expect(String(byId?.bookId)).toBe('fantasy402');
  ...
  test('maglive on buckeye host is rejected', () => {
    const db = openEventStore({ dbPath: ':memory:' });
    ensurePartnerRegistrySchema(db);
    upsertPartner(db, {
      id: 'partner-x',
  ...
  test('ace host allows ultralive; rejects plive', () => {
    const db = openEventStore({ dbPath: ':memory:' });
    ensurePartnerRegistrySchema(db);
    upsertPartner(db, {
      id: 'partner-ace',
    ...
    expect(row?.skinId).toBe('ace');
    expect(row?.mapper).toBe('unmapped');
    expect(row?.adapterId).toBe('unmapped');
  });
});
