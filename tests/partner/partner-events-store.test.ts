...
import {
  migratePartnerEventsInventoryIdentity,
  openEventStore,
...
describe('partner_events store', () => {
  ...
  test('upsertPartnerLiveEvents stamps skin/book/inventory product once per stream', () => {
    ...
    const row = db
      .query(
        `SELECT home, away, last_updated AS lastUpdated, sport, skin_id AS skinId,
         ...
         FROM partner_events WHERE stream_id = '100'`
      )
      .get() as {
      home: string;
      away: string;
    ...
    const count = (
      db.query(`SELECT COUNT(*) AS c FROM partner_events WHERE stream_id = '100'`).get() as {
        c: number;
      }
    ).c;
    expect(count).toBe(1);
  ...
  test('migrate deletes Test League fixture and backfills fantasy402 rows', () => {
    ...
    const leftover = (
      db.query(`SELECT COUNT(*) AS c FROM partner_events WHERE stream_id = '1'`).get() as {
        c: number;
      }
    ).c;
    ...
    const row = db
      .query(
        `SELECT skin_id AS skinId, book_id AS bookId, inventory_live_product AS inv
         FROM partner_events WHERE stream_id = '999'`
      )
