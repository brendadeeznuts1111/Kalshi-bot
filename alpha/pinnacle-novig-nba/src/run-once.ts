 ...
 *   bun src/run-once.ts --ticker=KXNBAGAME-26JAN15LALBOS --price=55 --sport=basketball_nba
 *   bun src/run-once.ts --ticker=KXNBAGAME-26JAN15LALBOS --fetch-book --offline
 */
...
const FIXTURE = joinPath(
  import.meta.dir,
  "../../../src/alpha/fixtures/basketball_nba-odds.json",
);
...
async function loadEvents(offline: boolean, sport: string) {
  if (offline) {
    ...
    return parseOddsEventsWire(wire);
  }
...
if (import.meta.main) {
  ...
  const sport = arg("sport") ?? "basketball_nba";
  ...
  if (!ticker || (!priceArg && !fetchBook)) {
    console.error(
      "Usage: bun src/run-once.ts --ticker=KXNBAGAME-... (--price=55 | --fetch-book) [--offline] [--live]",
    );
    process.exit(1);
  }
