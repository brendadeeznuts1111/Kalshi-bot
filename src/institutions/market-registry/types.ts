...
export type SportsSourceRegistry = {
  ...
  /**
   * Unified team/competitor registry (§73) — OPTIONAL until a source ships
   * team data. Each row carries the canonical team key + per-source id
   * mappings + optional image metadata. Generated by the same bake/check
   * machinery as the rest of the registry; absent = no source provides
   * teams yet (schema stays v1-compatible: teams is an optional array).
   */
  teams?: readonly TeamRegistryEntry[];
};

/** Unified team identity: canonical key + per-source ids + optional image. */
export type TeamRegistryEntry = {
  /** Canonical unified key, e.g. "team:atp:nadal" or "team:kalshi:CHI-BEARS". */
  team: string;
  /** Sport the team belongs to (a SportKey from registry.sports). */
  sport: string;
  /** Human label (the team name). */
  label: string;
  /** Per-source id mappings: source key -> that source's id for this team. */
  sourceIds: ReadonlyArray<{ source: string; id: string }>;
  /** Optional image metadata (filled when a source provides logo bytes). */
  image?: {
    logoUrl?: string;
    width?: number;
    height?: number;
...
export type SportsSourceRegistryArtifact = {
  ...
  /** Optional unified team registry (§73) — absent until a source ships teams. */
  teams?: ReadonlyArray<{
    team: string;
    sport: string;
    label: string;
