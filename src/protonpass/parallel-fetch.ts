...
import { SecretTelemetry } from "./telemetry.ts";
...
export async function fetchSecret(
  ...
  try {
    ...
    if (result.timedOut) {
      ...
      telemetry?.record({ uri, durationMs: r.durationMs, status: "error", error: r.error, fromCache: false });
      return r;
    }
    ...
    telemetry?.record({ uri, durationMs: r.durationMs, status: "error", error: msg, fromCache: false });
    return r;
  }
}
