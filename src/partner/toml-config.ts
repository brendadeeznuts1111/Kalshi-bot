 ...
 *
 * ## What --seed does
 * Upserts into SQLite tables `partners` + `betting_accounts` (live-product
 * capacity lives in `meta_json.liveProducts`, not a separate table). Capacity is
 * **computed at read time** by `computeProviderCapacity` — no denormalized
 * capacity_cache (by design).
 *
 ...
 *   4. desk URL       DESK_DOMAIN → SKINS Ultra default (host → SkinId)
 *
...
export const PARTNER_ENV_KEYS = [
  ...
  'PASSWORD',
  'DOMAIN',
  /** Live-product / Ultra wire (ezlive, dark, 2) — not white-label SkinId */
  'LIVE_PRODUCT',
  'CURRENCY',
] as const;
 ...
 * book-level uses DESK_DOMAIN only (retired bare-book DOMAIN envs ignored).
 */
export function resolvePartnerEnv(
  ...
  for (const key of keys) {
    for (const step of chain) {
      ...
      // itself — still never read retired bare-book DOMAIN; only DESK_DOMAIN.
