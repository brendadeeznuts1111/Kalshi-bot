...
import { maybeGzip } from "../lib/http-gzip.ts";
...
async function oddsReportResponse(req: Request): Promise<Response> {
  ...
  if (cached !== undefined && nowMs < cached.expiresAtMs) {
    ...
    try {
      ...
      // Conventional book logos: keys with a baked /assets/books/<key>.png
      // light up the report's Logo column without config edits.
      const bookLogoKeys = new Set(
        [...new Bun.Glob("*.png").scanSync({ cwd: joinPath(ROOT, "public/assets/books"), onlyFiles: true })]
          .map((f) => f.replace(/\.png$/, "")),
      );
      markdown = buildOddsReportMarkdown({
        ...
        ...(events.length > 0 ? { books: booksQuoting(config, events, bookLogoKeys) } : {}),
  ...
  const body = asHtml
    ...
    : markdown;
  // Gzip only when the client advertises Accept-Encoding: gzip (§240) — direct
  // res.json()/res.text() consumers keep the plain body.
  ...
  const { body: b, headers } = acceptsGzip ? maybeGzip(body, baseHeaders) : { body, headers: baseHeaders };
...
export function createResearchServer(options: ServeOptions = {}) {
  ...
  const serveOptions = {
    ...
    routes: {
      ...
      // Book logo swatches (baked by `bun run book:logos`) — conventional
      // /assets/books/<key>.png path resolved by the bookmaker store.
      "/assets/books/*": { dir: joinPath(ROOT, "public/assets/books") },
      "/partner-dashboard/*": { dir: joinPath(ROOT, "public/partner-dashboard") },
      // Sitemap/robots (regenerated by `bun run sitemap:gen` from the served
      // route set + odds registry; §239 real-gap close).
      "/sitemap.xml": new Response(Bun.file(joinPath(ROOT, "public/sitemap.xml")), {
        headers: { "content-type": "application/xml", "cache-control": "public, max-age=3600" },
      }),
      "/robots.txt": new Response(Bun.file(joinPath(ROOT, "public/robots.txt")), {
