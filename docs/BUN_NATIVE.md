...
| `Bun.semver.satisfies` | `src/research/bun-native.ts`, `tools/massey-sync-cli.ts`, `tools/massey-crossref-cli.ts` | `assertBunAtLeast` runtime gate; `Bun.semver.order` + `normalizeSemver` in signal-pipeline.ts (docs-drift, §121 — normalize ragged feeds first; order() throws on invalid) — CLI entries that need Bun 1.4-only APIs (WebView, cron) fail fast with a clear upgrade message instead of a ReferenceError.
