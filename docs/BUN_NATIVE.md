...
| Match liquidity db watch | `fs.watch` on `event-store.db` (+ WAL) → debounced ground rebuild | [`match-liquidity-db-watch.ts`](../tools/match-liquidity-db-watch.ts), `bun run liquidity:ground:watch-db` |
