...
`massey:edge-flags` — automatic edge flags (Massey implied vs live book odds): `computeEdgeFlags` engine + md/json formatters (threshold-configurable, tested); `odds-ticks-store` implements the live-odds persistence contract (`skin_events.odds_event_id` → `odds_ticks`, sides `home`/`away`, latest tick per side); CLI writes `research/outputs/massey-edge-flags.{md,json}`; cron runs it after crossref. Honest gap: the Pandora WS → coefficient-store live capture is in-memory only — nothing persists it to `odds_ticks` yet (tennis-history CSVs write `winner`/`loser` sides under the canonical `events` corpus, outside this join), so the CLI reports 0 priced events until the capture lands.
