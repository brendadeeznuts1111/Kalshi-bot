...
>
> **Current contract status: verify:contracts 59/59** (see docs/BUN_GROUNDED.md
...
- §170 — Per-module shape report — docs/BUN_GROUNDED.md (2026-08-24)
...
- §174 — Repo API grounded in the shape — docs/BUN_GROUNDED.md (2026-08-24)
...
- §229 — Bun.Terminal PTY pin wired — alpha:cluster --pty-pin renders styled output in a real PTY, graceful when the env denies PTYs; docs/BUN_TERMINAL.md (2026-08-27)
- §230 — BuildArtifact.stream() type-confusion pin — #10004/PR #33144 FIXED in runtime 34cbb9a40 (>= fix 7f33321f); S01c probe-locked (2026-08-27)
- §231 — Web Streams fixes probed — #25484/#29229/#37692/#33782 verified; #32640 "negative write()" contradicted (Promise<number>); streams.test.ts + BUN_STREAMS_TLS_WS (2026-08-27)
- §232 — Backpressure pinned — Bun.serve pauses ReadableStream bodies for slow/stalled clients (P5: 83 pulls/8 reads, 0 during stall); benchmarks 📋 in BUN_STREAMS_TLS_WS (2026-08-27)
- §233 — Backpressure enhanced — P6-P8: file/Blob streams, CompressionStream pipeline source pause (63 pulls), upload pause (30 pulls); probe 11/11 (2026-08-27)
- §234 — Bun.spawn pipe backpressure pinned — child blocks on the OS pipe (dd 64MiB, still running after 8 slow reads), full delivery + exit 0; probe 13/13 (2026-08-27)
- §235 — Bun.color 'ansi' ENV-driven, not TTY-driven — env vars pick depth even piped; sandbox env contaminated earlier probes; ansi:probe 26/26 + styledRGB (2026-08-27)
...
- bun:breaking-audit (tools/bun-breaking-audit.ts) encodes section 16:
  ...
  changes' checks).
- bun:claims-audit (tools/ground-check.ts (retired 2026-08-26) + src/lib/claims-audit.ts)
...
## Historical lesson bodies (removed 2026-08-26)

The §12–§178 chronological lesson bodies (≈6,000 lines of per-lesson probe
narratives) were condensed away during the grounding refactor. The **section
index above preserves every lesson title**, and all repo-wide `§N` references
resolve to it. Full bodies are preserved in git history (the commit that
removed them). Bun-API claim details now live in the generated
[`BUN_GROUNDED.md`](BUN_GROUNDED.md) and the curated [`BUN_FILE.md`](BUN_FILE.md),
[`BUN_SHELL.md`](BUN_SHELL.md), [`BUN_NATIVE.md`](BUN_NATIVE.md), and
[`BUN_BUILD_FINDINGS.md`](BUN_BUILD_FINDINGS.md) deep dives. The topical
pitfall catalog (§1–§11) above remains the operational record.
