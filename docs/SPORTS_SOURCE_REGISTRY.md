...
- Event type and participant format must be query-layer filters when the source selector exposes those dimensions. Kalshi competition selectors do. Polymarket currently exposes sport-tag inventory with `eventType` and `participantFormat` unresolved; consumers must quarantine format-sensitive matching until its adapter provides a parsed discriminator. Never infer singles/doubles/tournament compatibility post hoc from a successful name match.
