...
### Reference docs
- [`docs/AGENT-PITFALLS.md`](docs/AGENT-PITFALLS.md) — read-before-editing catalog of the harness/tooling failure modes (run_code lexer, edit/write policy, bash/git, bun:test, runtime-probe discipline).
- [`docs/CONNECTION-MASTERY.md`](docs/CONNECTION-MASTERY.md) — verified multi-protocol guide (runtime probes + official doc citations; corrections vs. common AI summaries: h2 not auto, `--experimental-http3-fetch`, WebTransport absent, WS `{signal}` unverified, `Bun.connect` requires `socket`).

### Feed adapters + protocol matrix
...
- Bun multi-protocol matrix (verified empirically on 1.4.0, 2026-08): plain `fetch` negotiates **http/1.1 by default** — HTTP/2 requires `--experimental-http2-fetch` (or `BUN_FEATURE_FLAG_EXPERIMENTAL_HTTP2_CLIENT=1`) **or the per-request `protocol: "http2"` option over https** (requires TLS ALPN; plaintext h2c throws `HTTP2Unsupported` — §14); HTTP/3 is `--experimental-http3-fetch` (honor Alt-Svc). `Bun.udpSocket` and `fetch(url, { unix })` are real but unused (no consumer). `Bun.connect` with `tls`/`socket.tcpNoDelay` real (no raw-TCP feed). Client `new WebSocket(url, { signal })` constructs but signal honoring is unverified — the manager uses `session.close()` for abort. **`WebTransport` is NOT present** in this build. Adopted: `retryableConnect` (universal AbortSignal retry wrapper, injectable sleep) — the unified error/abort/retry pattern the docs describe, tested.
