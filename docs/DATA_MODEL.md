...
### Reference docs
- [`docs/AGENT-PITFALLS.md`](AGENT-PITFALLS.md) — read-before-editing catalog of the harness/tooling failure modes (run_code lexer, edit/write policy, bash/git, bun:test, runtime-probe discipline).
- [`docs/CONNECTION-MASTERY.md`](CONNECTION-MASTERY.md) — verified multi-protocol guide (runtime probes + official doc citations; corrections vs. common AI summaries: h2 not auto, `--experimental-http3-fetch`, WebTransport absent, WS `{signal}` unverified, `Bun.connect` requires `socket`).
